...
import pandas as pd
from logging_helpers.setup_logger import Messenger, cleanup_logger, setup_logger
...
def process_file(
    ...
    messenger.info(f"--- Starting processing for buffer {uid} ---")
    t1 = time.perf_counter()

    df_clean, report, _ = clean_file(parquet_path, ROOT_DIR, plot_destination)

    messenger.info("Dumping settings")
    dump_settings(
    ...
    messenger.info("Saving processing report")
    save_cleaning_report(
        uid,
        report,
    ...
    messenger.info(
        f"--- Completed processing buffer {uid} in {processing_time:.3f} s ---"
    )
...
def process_directory(
    ...
    messenger.info("Intiating directory processing...")
    ...
    for PARQ_PATH in PARQ_PATHS[n_start:n_end]:
        messenger.info(f"Processing {PARQ_PATH.name}")
    ...
    messenger.info("Completed directory processing!")
...
if __name__ == "__main__":
    ...
    BASE_PATH = Path(
        "C:/Users/Neutron Computer/Documents/Data/20220922_AmBedistancetests/"
    )
    ...
    for ROOT_DIR in BASE_PATH.iterdir():
        ...
        # 0 and 30 cm bad...
        process_directory(ROOT_DIR / "raw_data/parquet", plot_destination=PLOT_PATH)
