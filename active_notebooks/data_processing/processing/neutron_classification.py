...
def generate_new_neutron_window(
    ...
    energy_bin_midpoints = np.ndarray([
        (lb+ub)/2 for lb, ub 
        in zip(energy_bin_left_edges, energy_bin_right_edges)
    ])
    ...
    bottom_border = _generate_new_window_bottom_border(slice_fit_df, energy_bin_left_edges, sigma)
    top_border = _generate_new_window_top_border(slice_fit_df, energy_bin_left_edges, sigma)
...
def _generate_new_window_bottom_border(
    slice_fit_df: pd.DataFrame, 
    energy_bin_left_edges: np.ndarray, 
    sigma: float
